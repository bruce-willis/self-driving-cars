...
def get_session_and_ops(batch_size, num_point, model_module, model_path):
    ...
    with tf.Graph().as_default():
        device_id = '/device:GPU:0' if tf.test.is_gpu_available(
        ) else '/device:CPU'
...
@dataclass
class FrustumInferenceResult:
    center: List[float]
    heading_class: List[int]
    heading_result: List[float]
    size_class: List[int]
    size_result: List[float]
...
def inference(sess, ops, pc, one_hot_vec, batch_size, num_classes):
    ...
    for i in range(num_batches):
        feed_dict = {
            ops['pointclouds_pl']:
            pc[i * batch_size:(i + 1) * batch_size, ...],
            ops['one_hot_vec_pl']:
            one_hot_vec[i * batch_size:(i + 1) * batch_size, :],
            ops['is_training_pl']:
            False
        }
        ...
        heading_logits[i * batch_size:(i + 1) * batch_size,
                       ...] = batch_heading_scores
        heading_residuals[i * batch_size:(i + 1) * batch_size,
                          ...] = batch_heading_residuals
        size_logits[i * batch_size:(i + 1) * batch_size,
                    ...] = batch_size_scores
        size_residuals[i * batch_size:(i + 1) * batch_size,
    ...
    return [
        FrustumInferenceResult(center, heading_c, heading_r, size_c, size_r)
